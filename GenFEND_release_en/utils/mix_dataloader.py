...
bge_model = SentenceTransformer('./pretrained_model/bge-large-en-v1.5') # your bge-large-en-v1.5 file path


class GossipCopDataset(Dataset):
    def __init__(self, path_to_file, emb_type, shuffle, use_endef, aug_prob):
        ...
        if emb_type == 'bge':
            self.cmt_emb_type = 'bge'
            self.cmt_emb_dir = '/bge-large-en-v1.5'
    ...
    def __getitem__(self, index):
        ...
        new_item = {'title': item['title'], 'content': item['content'], 'comment_fea': comment_feature, 
                    'flag': flag, 'cmt_emb_type': self.cmt_emb_type,
                     'cmt_fea': gen_comment_feature,
                    'male_cmt_fea': gen_comment_feature[self.male_index], 'female_cmt_fea': gen_comment_feature[self.female_index],
...
def my_collate(batch):
    ...
    for i in range(len(batch)):
        empty_cmt_fea = bge_model.encode("")
        empty_cmt_fea = np.expand_dims(empty_cmt_fea, axis=0)
        content_token_ids, content_masks = word2input(batch[i]['title'] + '[SEP]' + batch[i]['content'], max_len = 200)
...
def my_collate2(batch):
    ...
    edu3_cmt_fea_batch = []
    for i in range(len(batch)):
        if batch[i]['cmt_emb_type'] == 'bge':
            comments_empty_fea = bge_model.encode("")
        comments_empty_fea = np.expand_dims(comments_empty_fea, axis = 0)
        content_token_ids, content_masks = word2input(batch[i]['content'], max_len = 300)
        content_token_ids_batch.append(content_token_ids)
        content_masks_batch.append(content_masks)
        label_batch.append(batch[i]['label'])
...
def get_dataloader(path, batch_size, no_comment, emb_type, shuffle, use_endef = False, aug_prob = 0.0):
    dataset = GossipCopDataset(path, emb_type, shuffle, use_endef, aug_prob)
    if no_comment:
