...
from utils.utils import data2gpu_mix, Averager, metrics, Recorder, kl_dist
from utils.mix_dataloader import get_dataloader

class dEFENDGenFENDModel(torch.nn.Module):
    def __init__(self, cnt_emb_dim, cmt_emb_dim, dropout):
        super(dEFENDGenFENDModel, self).__init__()
        self.cnt_emb_dim = cnt_emb_dim
        self.cmt_emb_dim = cmt_emb_dim
        self.dropout = dropout
        self.fea_dim = 768
        self.mlp_dims = [self.fea_dim * 2, self.fea_dim, 384]
        self.mtispp_module = MultiSubppModel(self.cnt_emb_dim, self.cmt_emb_dim, self.dropout)

        self.cnt_bert = BertModel.from_pretrained('bert-base-uncased').requires_grad_(False)
        ...
        self.cmt_mlp = MLP(self.cmt_emb_dim, [self.fea_dim], dropout, output_layer = False)
        self.co_att = ParallelCoAttentionNetwork(self.fea_dim, self.fea_dim)
        self.mlp = MLP(self.fea_dim * 3, self.mlp_dims, dropout, output_layer = True)
    
    def forward(self, **kwargs):
        content = kwargs['content']
        content_masks = kwargs['content_masks']
        real_cmt_emb = kwargs['comments']
        real_comments_masks = kwargs['comments_masks']

        cnt_fea = self.cnt_bert(content, attention_mask = content_masks)[0]
        cmt_fea = self.cmt_mlp(real_cmt_emb)
        _, _, cnt_fea, real_cmt_fea= self.co_att(cnt_fea.permute(0, 2, 1), cmt_fea, content_masks, real_comments_masks)

        kwargs['cnt_fea'] = cnt_fea
        gen_cmt_fea, gate_value = self.mtispp_module(**kwargs)

        final_fea = torch.cat((cnt_fea, real_cmt_fea, gen_cmt_fea), dim = -1)
...
class Trainer():
    ...
    def train(self):
        self.model = dEFENDGenFENDModel(self.config['cnt_emb_dim'], self.config['cmt_emb_dim'], self.config['model']['mlp']['dropout'])
        ...
        for epoch in range(self.config['epoch']):
            ...
            if mark == 'save':
                ...
                torch.save(self.model.state_dict(),
                        os.path.join(self.save_path, 'parameter_defendmultisppmodel_' + str(best_metric) + '.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        self.model.load_state_dict(torch.load(os.path.join(self.save_path, 'parameter_defendmultisppmodel_' + str(best_metric) + '.pkl')))
        ...
        return test_results, os.path.join(self.save_path, 'parameter_defendmultisppmodel_' + str(best_metric) + '.pkl')
    ...
    def test(self, dataloader):
        ...
        for i in range(len(pred)):
            results.append({"label": label[i], "pred": pred[i]})
        with open(os.path.join(self.results_analysis_path, 'results_test.json'), 'w', encoding = 'UTF-8') as f:
